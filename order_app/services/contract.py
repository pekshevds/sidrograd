...
from order_app.models import Contract, Point
...
def address_by_id(address_id: str) -> Contract:
    return Point.objects.filter(id=address_id).first()


def handle_contract(contract_dir: dict) -> Contract:
...
def handle_contract_list(contract_list: list) -> QuerySet[Contract]:
    contract_id = []
